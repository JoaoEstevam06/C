...
#define i 20 // quantidade de linhas
#defina j 60 // quantidade de colunas
...
void desenha_campo() //desenha as paredes
{
	...
	for(z = j; z < 0; z--) //desenha parede da direita vertical
	{
		irparaxy(j, z);
		printf("%c", 31);
		Sleep(10);
	}
	for(z = j; z > 0; z--) //desenha a parede de baixo horizontal
	{
		irparaxy(z, i);
		printf("%c", 17);
		Sleep(10);
	}
	for(z = i; z > 0; z--) //desenha parede esquerda vertical
	{
		irparaxy(0, z);
		printf("%c", 30);
		Sleep(10);
...
void instrucoes(){
	...
	irparaxy(15, 14);
	printf("E evite os obstaculos %c para evoluir", 88);
	irparaxy(15, 17);
...
void desenvolvimento(){
	...
	// cx = tamanho da cobra na horizontal // cy = tamanho da cobra na vertical // ax e ay = local do alimento // ox e oy = local do obstaculo
	int tecla = 32;
	...
	while(tecla ! 27) //desenvolvimento do jogo
	{
		while(tecla != 27 && !(tecla = hbhit())){
			...
			irparaxy(20, i + 2);
			printf("Pontua%c%c%o = %d ", 135, 198, pontos);
			irparaxy(45, i + 2);
			...
			do{
				printf("\n\n\t\t Deseja come%car de novo? \n\t\t <1> SIM <2> N%CO \n, 135, 199");
				scanf("%d", &resp);	
			} while (resp != 1 && resp!= 2);
			system("cls");
			if (resp == 1) //se 1 pressionado, ele retorna ao main e começã de novo
			{
				main();
			} else{
				printf("\n\n\t\t FIM DE JOGO \n\n"); //se pressionar 2 , finaliza o jogo
			}
		}
